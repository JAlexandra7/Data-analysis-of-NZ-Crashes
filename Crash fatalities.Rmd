...
# Loading Data
```{r}
Crash <- read.csv("Crash_Analysis_System_(CAS)_data.csv", na.strings = c("", "Unknown", "Null", "Nil"))
...
```{r}
Crash <- subset(Crash, select = c(-crashSeverity, -crashLocation1, -crashLocation2, -crashFinancialYear, -tlaName, -tlaId, -directionRoleDescription, -seriousInjuryCount, -minorInjuryCount, -urban))
```
...
The data set is made up of majority count data and categorical variables.
...
Crash[,1:30] %>% missing_plot(title = "Missing data by observation and variable")
```

```{r}
Crash[,31:59] %>% missing_plot(title = "Missing data by observation and variable")
```
...
```{r}
missing.table <- do.call(rbind, Map(data.frame, variable = missing_percents_colnames, percentage = percentages))
...
areaUnitID is strongly correlated with meshblockID with a correlation of 1.
...
fatalCount the response variable has a skewness of 15.07 this is significant departure from the normal distribution which has a skewness of 0.
...
fatalCount the response variable has a kurtosis of 363.7, which means it has a leptokurtic distribution (high peak) this is significant departure from the normal distribution where the absolute kurtosis value should not exceed 7.1.

taxi, motorcycle, bus, moped, otherVehicleType, schoolBus and unknownVehicleType all have extremely high kurtosis values, indicating leptokurtic distributions.
...
```{r}
model_glm_P <- glm(fatalCount ~ speedLimit + motorcycle + roadLane + truck + region + NumberOfLanes + otherVehicleType + crashSHDescription + carStationWagon + flatHill, data = Crash2, family = poisson)
summary(model_glm_P)
```
...
```{r}
model_nb <- glm.nb(fatalCount ~ speedLimit + motorcycle + roadLane + truck + region + NumberOfLanes + otherVehicleType + crashSHDescription + carStationWagon + flatHill, data = Crash2)
```
