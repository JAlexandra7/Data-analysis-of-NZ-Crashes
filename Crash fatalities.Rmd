...
```{r}
# Saving plots:
# plotA <- missing_plot(Crash[,1:30], title = "Missing data by observation and variable")
# plotB <- missing_plot(Crash[,31:59], title = "Missing data by observation and variable")
# ggsave(plotA, 
...
The KS test is not statistically significant at a significance level of 0.05
...
The dispersion test is statistically significant at a significance level of 0.05, I may need to consider additional methods to deal with overdispersion.
...
```{r}
model.diag.metrics <- augment(model_nb)
...
```
There are 208 observations that have standardized residuals larger than 3 in absolute value.

I am going to remove these from the data set:
```{r}
Crash2 <- Crash2[-c(large_std_resid$index), ]
```
...
Stepwise BIC:
```{r}
step(model_nb, direction = "both", k = log(nrow(Crash2)))
```
...
The KS test and dispersion test are not statistically significant at a significance level of 0.05, so I conclude that the data does fit a negative binomial model.
...
outliers
```
There are 1153 more outliers than expected in the data set.

Checking the residual plots to see if removing these outliers fixes the outlier problem:
```{r}
Potential_outliers <- sort(simulationOutput2$scaledResiduals, decreasing = TRUE)[1:1153]
Crash3 <- Crash2[-c(Potential_outliers), ]
```

```{r}
model_nb2 <- glm.nb(fatalCount ~ speedLimit + motorcycle + roadLane + truck + region + 
                     NumberOfLanes + otherVehicleType + crashSHDescription + 
                     carStationWagon + flatHill, data = Crash3)
```
...
```{r}
outliers3 <- testOutliers(simulationOutput3, type = "bootstrap", nBoot = 20, plot = FALSE)
```
...
Removing the outliers identified by the first outlier test did not fix the outlier issue, given this result I have decided to keep the outliers in the data set and to continue with my original negative binomial model.
...
For a one unit change in the speedLimit, the log of expected counts of fatalCount changes by 0.03264, given that the other predictor variables in the model are held constant. This change is statistically significant at a significance level of 0.05

For a one unit change in the motorcycle, the log of expected counts of fatalCount changes by 1.048, given that the other predictor variables in the model are held constant. This change is statistically significant at a significance level of 0.05

The expected log count for 2-way road lane is 1.408 higher than the expected log count for a 1-way road lane, given that the other predictor variables in the model are held constant. This is statistically significant at a significance level of 0.05

The expected log count for Off road lane is 1.593 higher than the expected log count for a 1-way road lane, given that the other predictor variables in the model are held constant. This is statistically significant at a significance level of 0.05

For a one unit change in the truck, the log of expected counts of fatalCount changes by 0.6753, given that the other predictor variables in the model are held constant. This change is statistically significant at a significance level of 0.05

The expected log count for the Bay of Plenty region is 0.4476 higher than the expected log count for the Auckland region, given that the other predictor variables in the model are held constant. This is statistically significant at a significance level of 0.05

The expected log count for the Canterbury region is 0.3569 higher than the expected log count for the Auckland region, given that the other predictor variables in the model are held constant. This is statistically significant at a significance level of 0.05

The expected log count for the Gisborne region is 0.1109 higher than the expected log count for the Auckland region given that the other predictor variables in the model are held constant. This is not statistically significant at a significance level of 0.05

The expected log count for the Hawke's Bay region is 0.1867 higher than the expected log count for the Auckland region given that the other predictor variables in the model are held constant. This is statistically significant at a significance level of 0.05

The expected log count for the Manawatū-Whanganui region is 0.3259 higher than the expected log count for the Auckland region, given that the other predictor variables in the model are held constant. This is statistically significant at a significance level of 0.05

The expected log count for the Marlborough region is 0.257 higher than the expected log count for the Auckland region, given that the other predictor variables in the model are held constant. This is statistically significant at a significance level of 0.05

The expected log count for the Nelson region is -0.2016 lower than the expected log count for the Auckland region, given that the other predictor variables in the model are held constant. This is not statistically significant at a significance level of 0.05

The expected log count for the Northland region is 0.3716 higher than the expected log count for the Auckland region, given that the other predictor variables in the model are held constant. This is statistically significant at a significance level of 0.05

The expected log count for the Otago region is -0.1064 lower than the expected log count for the Auckland region, given that the other predictor variables in the model are held constant. This is not statistically significant at a significance level of 0.05

The expected log count for the Southland region is 0.03776 higher than the expected log count for the Auckland region, given that the other predictor variables in the model are held constant. This is not statistically significant at a significance level of 0.05

The expected log count for the Taranaki region is 0.2153 higher than the expected log count for the Auckland region, given that the other predictor variables in the model are held constant. This is statistically significant at a significance level of 0.05

The expected log count for the Tasman region is -0.09333 lower than the expected log count for the Auckland region, given that the other predictor variables in the model are held constant. This is not statistically significant at a significance level of 0.05

The expected log count for the Waikato region is 0.3603 higher than the expected log count for the Auckland region, given that the other predictor variables in the model are held constant. This is statistically significant at a significance level of 0.05

The expected log count for the Wellington region is -0.1435 lower than the expected log count for the Auckland region, given that the other predictor variables in the model are held constant. This is statistically significant at a significance level of 0.05

The expected log count for the West Coast region is 0.2149 higher than the expected log count for the Auckland region, given that the other predictor variables in the model are held constant. This is statistically significant at a significance level of 0.05

For a one unit change in the NumberOfLanes, the log of expected counts of fatalCount changes by -0.1688, given that the other predictor variables in the model are held constant. This change is statistically significant at a significance level of 0.05

For a one unit change in the otherVehicleType, the log of expected counts of fatalCount changes by 0.3191, given that the other predictor variables in the model are held constant. This change is statistically significant at a significance level of 0.05

For a one unit change in the crashSHDescriptionYes, the log of expected counts of fatalCount changes by 0.1878, given that the other predictor variables in the model are held constant. This change is statistically significant at a significance level of 0.05

For a one unit change in the carStationWagon, the log of expected counts of fatalCount changes by -0.2145, given that the other predictor variables in the model are held constant. This change is statistically significant at a significance level of 0.05

For a one unit change in the flatHillHill, the log of expected counts of fatalCount changes by -0.02383, given that the other predictor variables in the model are held constant. This change is not statistically significant at significance level of 0.05

# Conclusion

The coefficients of the the regions Gisborne, Marlborough, Nelson, Otago, Southland and Tasman aren't statistically significant which means that there is no statistically significant difference in the count of deaths for a car crash (fatalCount) between those regions and the reference level Auckland. However other regions 
...
For future projects I will split my data into a training set and a testing set so that I can use model evaluation metrics such as AUC-ROC, accuracy, precision and recall.
